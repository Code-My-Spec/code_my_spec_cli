...
MCP tool for listing Google Analytics 4 custom metrics for a specified property.

## Functions

### execute/1

Execute the list custom metrics tool with the provided arguments.
...
4. Call Analytics Admin API to retrieve custom metrics list
5. Format response with custom metrics array and pagination info
...
- successfully lists custom metrics with valid property_id
- respects page_size parameter when provided
- handles pagination with page_token
- returns empty list when no custom metrics exist
...
**Process**:
1. Extract custom metrics array from API response
2. Map each metric to simplified format with key fields
3. Extract pagination metadata (next_page_token)
4. Build output map with metrics list and pagination info
5. Return formatted response map

**Test Assertions**:
- formats empty response correctly
- formats single custom metric correctly
- formats multiple custom metrics correctly
- includes all required metric fields
